...
- **ziin analysis engine** ([post-from-sources](../post-from-sources/SKILL.md)): 핵심 팩트 → 배경 동인 → 시나리오 → 유의점 → 지인의 판단 → **개념 노트(에버그린)**.
- **해석·의견 중심**: “야간선물 % → 오늘 방향” 수준의 단순 전달 금지. 갭·수급의 **원인**과 작성자 시나리오를 써야 한다.
- **현상:해석 = 1:1**, **팩트/해석 라벨 분리**.
- **Anti-AI 문체**: 인사·개요·불렛3·요약 당부 골격 금지. 1인칭 분석가 어조.
- **암호형 트레이더 속어 금지** — 전문용어 + 쉬운 설명.
- **핵심 숫자 5~7개**만 표로; 본문 숫자 나열 금지.
- **시나리오**: 갭 유지 vs 갭 메움 (또는 상승/중립/하락) + 관찰 가능한 수급 조건.
- Mode B Hangul 본문 **≥ ~1,200자** (권장 1,500+).
- **확인되지 않은 지수·%·금액 금지.**
- **한국어** 톤. AI·생성 언급 금지(본문). frontmatter `aiAssisted: true` 유지.
- **제목:** 검색 친화형. [Title Policy](../post-from-sources/SKILL.md#title-policy-search-friendly--naver-as-discovery-only) — `네이버`/`naver-*` 금지.
- 신규·개정 글에만 적용 (아카이브 일괄 재작성 금지).
...
### 오늘의 결론
질문 + 해석이 담긴 2~4문장 (야간선물%→방향만 쓰지 말 것)
...
### 투자자 유의점
장 초반 30분 등 관찰 포인트 3개 (권유·목표가 금지)

### 지인의 판단
1인칭 고유 해석 3~6문장

### 개념 노트 (한 줄+)
오늘 테마와 연결된 원리 1개 (베이시스·프로그램 등)
...
**Body structure (daily):** post-from-sources **ziin analysis engine** 필수. 단순 시황 전달·AI 템플릿·암호 속어 금지. Hangul **≥ ~1,200자**(권장 1,500+).

1. 검색 친화형 `title`
2. `## 오늘의 결론` — 해석 포함 2~5문장
3. `## 핵심 팩트` — GFM 표 **5~7행**
4. `## 배경 동인` — 갭/변동 **원인** + 현상:해석 1:1
5. `## 테마 심화` (선택)
6. `## 데이터 시나리오` — 갭 유지 vs 갭 메움 (+ 수급 관찰값)
7. `## 투자자 유의점` — 장 초반 30분 등 (권유 금지)
8. `## 지인의 판단` — **필수** 1인칭
9. `## 개념 노트` — **필수** 에버그린 개념 1블록
10. `## 결론` + 짧은 면책
11. `## 출처`
...
- [ ] **ziin engine:** 결론 · 핵심 팩트 · 배경 동인 · 시나리오 · 유의점 · 지인의 판단 · **개념 노트**
- [ ] Hangul ≥ ~1,200 (권장 1,500+); 현상:해석 1:1; “야간선물→방향” 단순 전달 없음
- [ ] Anti-AI 골격 없음; 1인칭 분석 톤; 검색 친화형 제목; 암호 속어 없음; 표 ≤7행
- [ ] 팩트/해석 라벨 분리; 균등 N대·일간 FAQ 없음
...
1. **기준일·시각대** (KST)
2. **오늘의 결론 한 줄** + 배경 동인 요지 + 시나리오(갭 유지/메움) 요약
